...
namespace GPUInstancer.CrowdAnimations
{
    public class GPUICrowdRuntimeData : GPUInstancerRuntimeData
    {
        ...
        private bool _isSyncroniousBoneUpdate;
        ...
        public GPUICrowdRuntimeData(GPUInstancerPrototype prototype) : base(prototype)
        {
            ...
            _isSyncroniousBoneUpdate = ((GPUICrowdPrototype)prototype).animationData.isSynchronousBoneUpdates;
        }
        ...
        {
            if (!_isSyncroniousBoneUpdate && waitingBoneDataRequest)
            {
    ...
    struct ApplyBoneUpdatesJob : IJobParallelForTransform
    {
        ...
        public void Execute(int index, TransformAccess transform)
        {
            ...
            Matrix4x4 localToWorld = bakedAnimationData[bakedDataIndex] * bindPoses[boneIndex].inverse;
            if (localToWorld.ValidTRS())
            {
                Vector4 position = localToWorld.GetColumn(3);
                Quaternion rotation = localToWorld.rotation;
                Vector3 scale = localToWorld.lossyScale;

                float weight = weightData.x;
                if (weight < 1) // blending
                {
                    position *= weight;
                    scale *= weight;
                    ...
                    for (int w = 1; w < 4; w++)
                    {
                        ...
                        if (weight > 0)
                        {
                            ...
                            localToWorld = bakedAnimationData[bakedDataIndex] * bindPoses[boneIndex].inverse;
                            if (localToWorld.ValidTRS())
                            {
                                position += localToWorld.GetColumn(3) * weight;
                                rotation = Quaternion.Lerp(rotation, localToWorld.rotation, weight);
                                scale += localToWorld.lossyScale * weight;
                            }
                        }
                    }
                }
