...
namespace GPUInstancer.CrowdAnimations
{
    public class GPUICrowdShaderBindings : GPUInstancerShaderBindingsExtension
    {
        ...
        public Shader GetInstancedShader(List<ShaderInstance> shaderInstances, string shaderName)
        {
            ...
            if (!shaderName.Equals(GPUICrowdConstants.SHADER_GPUI_CROWD_STANDARD_SPECULAR))
            {
                if (Application.isPlaying)
                    Debug.LogWarning("Can not find instanced shader for : " + shaderName + ". Using Standard shader instead.", Shader.Find(shaderName));
                return GetInstancedShader(shaderInstances, GPUICrowdConstants.SHADER_GPUI_CROWD_STANDARD_SPECULAR);
            }
        ...
        public Material GetInstancedMaterial(List<ShaderInstance> shaderInstances, Material originalMaterial)
        {
            if (originalMaterial == null || originalMaterial.shader == null)
            {
                ...
                return new Material(GetInstancedShader(shaderInstances, GPUICrowdConstants.SHADER_GPUI_CROWD_STANDARD_SPECULAR));
            }
